...
import (
	...
	"distributedKV/test-tool/labgob"
	"sync"
	"sync/atomic"
	"time"
)
...
func (rf *Raft) GetCommittedLogs() ([]LogEntry, int) {
	rf.mu.Lock()
	defer rf.mu.Unlock()
	if rf.lastIncludedIndex == rf.commitIndex {
		return []LogEntry{}, rf.commitIndex
	}
	return rf.log[:rf.commitIndex-rf.lastIncludedIndex], rf.commitIndex
